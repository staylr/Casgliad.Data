...
module internal ModeInfo =
    ...
    type ExtraGoals =
        { BeforeGoals: ResizeArray<Goal>
          AfterGoals: ResizeArray<Goal> }
        static member init() =
            { BeforeGoals = ResizeArray<Goal> ()
              AfterGoals = ResizeArray<Goal> () }

        member x.isEmpty() =
            x.BeforeGoals.Count = 0 && x.AfterGoals.Count = 0
    ...
    let unifyContextOfModeContext modeContext =
        ...
        | ModeContextCall (name, index) ->
            { MainContext = UnifyMainContext.CallArgUnify (name, index)
              SubContext = [] }
        | ModeContextUninitialized -> invalidOp "uninitialized context"

    let getContext (modeInfo: ModeInfo) = (modeInfo.CurrentSourceInfo, modeInfo)

    let setContext (goal: Goal) (modeInfo: ModeInfo) =
        ((),
         { modeInfo with
               CurrentSourceInfo = goal.Info.SourceInfo })

    let setCallContext (callee: Callee) (modeInfo: ModeInfo) =
        ((),
         { modeInfo with
               ModeContext = ModeContextCall (callee, 0) })
    ...
    let setCallArgContext (argNum: int) (modeInfo: ModeInfo) =
        let modeContext =
            ...
            | _ -> invalidOp "unexpected ModeContext for setCallArgContext"
    ...
    let unsetCallContext (modeInfo: ModeInfo) =
        ((),
         { modeInfo with
               ModeContext = ModeContextUninitialized })

    let getModeContext (modeInfo: ModeInfo) = (modeInfo.ModeContext, modeInfo)

    let haveErrors (modeInfo: ModeInfo) = (modeInfo.Errors <> [], modeInfo)

    let clearErrors (modeInfo: ModeInfo) = ((), { modeInfo with Errors = [] })

    let getErrors (modeInfo: ModeInfo) = (modeInfo.Errors, modeInfo)

    let getInstMap (modeInfo: ModeInfo) = (modeInfo.InstMap, modeInfo)

    let getDelayInfo (modeInfo: ModeInfo) = (modeInfo.DelayInfo, modeInfo)
    ...
    let getCalledRelationModeInfo (relationProcId: RelationProcId) (modeInfo: ModeInfo) =
        let modes =
            modeInfo.LookupRelationModes (fst relationProcId)
    ...
    let getCalledFunctionModeInfo (methodInfo: FSharpProcId) (modeInfo: ModeInfo) =
        let modes =
            modeInfo.LookupFSharpFunctionModes (fst methodInfo)
    ...
    let lookupVar v (modeInfo: ModeInfo) = (modeInfo.VarSet.Vars.[v], modeInfo)

    let cloneVar v (modeInfo: ModeInfo) =
        let var = modeInfo.VarSet.Vars.[v]

        let (varset, newVar) =
            modeInfo.VarSet.newNamedVar (var.Name, var.VarType)

        (newVar, { modeInfo with VarSet = varset })

    let getInstTable (modeInfo: ModeInfo) = (modeInfo.InstTable, modeInfo)
    ...
    let withLockedVars reason vars f (modeInfo: ModeInfo) =
        ...
        (res,
         { modeInfo'' with
               LockedVars = lockedVars0 })

    let modeError waitingVars error (modeInfo: ModeInfo) =
        let errorInfo =
            { ModeErrorInfo.Vars = waitingVars
              Error = error
              SourceInfo = modeInfo.CurrentSourceInfo
              ModeContext = modeInfo.ModeContext }

        ((),
         { modeInfo with
               Errors = List.append (modeInfo.Errors) [ errorInfo ] })
    ...
    let setVarInst (var: VarId) (newInst0: InstE) (maybeUnifiedInst: InstE option) modeInfo =
        let doSetVarInst newInst modeInfo =
            ...
            ((),
             { modeInfo with
                   InstMap = modeInfo.InstMap.setVar var newInst
                   DelayInfo = delayInfo })
        ...
        if not (modeInfo.InstMap.isReachable ()) then
            ...
            if (Bound NotReached = (modeInfo.InstTable.expand (newInst))) then
                ...
                ((),
                 { modeInfo with
                       InstMap = InstMap.initUnreachable })
                ...
                ((),
                 { modeInfo with
                      ...
                      InstMatch.instMatchesBinding
                          modeInfo.InstTable
                          newInst
                          oldInst
                          (Some varDefn.VarType)
                          InstMatch.AnyMatchesAny
                  )) then
                ...
                | Some (reason, _) ->
                    let error =
                        ModeErrorBindLockedVar (reason, var, oldInst, newInst)

                    modeError (TagSet.ofList [ var ]) error modeInfo
                | None -> doSetVarInst newInst modeInfo
            else
        ...
        state {
            if (inst = Bound NotReached) then
                ...
                | Bound (BoundCtor details) ->
                    ...
                    | [] -> do! setInstMap InstMap.initUnreachable
                    | [ boundInst ] ->
                        let boundArgsAndInsts =
                            List.zip3 argVars boundInst.ArgInsts unifyArgInsts

                        for (arg, boundArgInst, argInst) in boundArgsAndInsts do
                            do! setVarInst arg (Bound boundArgInst) (Some argInst)
                    | _ :: _ -> failwith "Expected single constructor in bindArgs"
        }
    ...
    let rec mergeVarInsts (instTable: InstTable) insts varType =
        ...
        | [ inst ] -> Some inst
        | [ inst1; inst2 ] -> instTable.mergeInst (inst1, inst2, varType)
        | _ ->
            let (list1, list2) =
                List.splitAt ((List.length insts) / 2) insts
            ...
            | (Some mergedInst1, Some mergedInst2) -> instTable.mergeInst (mergedInst1, mergedInst2, varType)
            | _ -> None
    ...
    let instMapMerge nonLocals (armInstMaps: (SourceInfo * InstMap) list) mergeContext modeInfo =
        let mergeInstOfVar (instMap: InstMap, errors) var =
            let varInsts =
                armInstMaps
                |> List.map (fun (_, armInstMap) -> armInstMap.lookupVar var)

            let varDefn = modeInfo.VarSet.Vars.[var]

            let maybeMergedInst =
                mergeVarInsts modeInfo.InstTable varInsts (Some varDefn.VarType)
            ...
            | None -> (instMap.setVar var (Bound NotReached), errors)

        let instMap0 = modeInfo.InstMap

        let reachableInstMaps =
            armInstMaps
            |> List.filter (fun (_, instMap) -> instMap.isReachable ())

        if (instMap0.isReachable () && reachableInstMaps <> []) then
            let (instMap, mergeErrors) =
                nonLocals
                |> TagSet.fold mergeInstOfVar (instMap0, [])
            ...
            | firstError :: _ ->
                let error =
                    ModeErrorMergeDisjunction (mergeContext, mergeErrors)
        ...
        let (instMapDelta, modeInfo'') =
            ...
            | Conjunction ([]) ->
                (InstMap.initReachable,
                 { modeInfo' with
                       InstMap = initialInstMap })
            | _ -> (InstMap.computeInstMapDelta initialInstMap modeInfo'.InstMap goal.Info.NonLocals, modeInfo')
    ...
    let withNoDelayOrExtraGoals f modeInfo =
        ...
        let modeInfo' =
            { modeInfo with
                  MayChangeCalledProc = false
                  CheckingExtraGoals = true }

        let (res, modeInfo'') = f modeInfo'

        (res,
         { modeInfo'' with
               MayChangeCalledProc = mayChangeCalledProc
               CheckingExtraGoals = false })
