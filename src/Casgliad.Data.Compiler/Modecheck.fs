...
module internal Modecheck =
    let getModeOfArgs (argInitialInsts: InstE list) (finalInst: BoundInstE) =
        ...
        | NotReached
        | Any
        | Ground -> pairWithFinalInst argInitialInsts finalInst
        | BoundCtor boundInsts ->
            match boundInsts.BoundInsts with
            | [] -> pairWithFinalInst argInitialInsts NotReached
            | [ singleFunctor ] -> List.map2 (fun i1 i2 -> (i1, i2)) argInitialInsts singleFunctor.ArgInsts
            | _ -> invalidOp "expected single functor in getModeOfArgs"
        | _ -> invalidOp $"unexpected inst in getModeOfArgs {finalInst}"
    ...
    let rec setVarInstListCall argNum vars initialInsts finalInsts (extraGoals: ExtraGoals) =
        state {
            ...
            | (var :: vars', initialInst :: initialInsts', finalInst :: finalInsts') ->
                ...
                return var' :: vars''
            | _ -> return invalidOp "setVarInstListCall"
        }
    ...
    and modecheckIfThenElse condGoal thenGoal elseGoal goalInfo =
        state {
            let! instMap0 = getInstMap
            let! condGoal' = withLockedVars VarLockReason.VarLockIfThenElse goalInfo.NonLocals (modecheckGoal condGoal)
            ...
            let! (thenGoal', thenInstMap) =
                if (condInstMap.isReachable ()) then
                    ...
                    state { return (succeedGoal, condInstMap) }
            ...
            let armInstMaps =
                [ (thenGoal.Info.SourceInfo, thenInstMap)
    ...
    and modecheckUnifyVarCtor lhs ctor args context goalInfo =
        let rec splitComplicatedSubUnifies
            (args0: VarId list)
            (modes: UnifyMode list)
            (argsRes: VarId list)
            ...
            state {
                ...
                | _ -> return invalidOp "length mismatch in splitComplicatedSubUnifies"
            }
    ...
    and handleExtraGoals
        ...
        (goalInfo0: GoalInfo)
        (goalExpr: GoalExpr)
        (initialInstMap: InstMap)
        (extraGoals: ExtraGoals)
        : ModeStateFunc<GoalExpr> =
        state {
