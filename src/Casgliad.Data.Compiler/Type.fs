...
module internal Type =
    let tupleConstructor tupleType =
        Tuple (FSharpType.GetTupleElements(tupleType).Length)
            ...
            if (concreteType.IsAssignableTo (case.DeclaringType)) then
                ...
                raise (
                    System.ArgumentException (
                        $"invalid union case inst for ${case.Name}: ${concreteType.FullName} is not an instance of ${case.DeclaringType.FullName}"
                    )
                )
