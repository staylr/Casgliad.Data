...
type FailingContext = { Context: SourceInfo; Goal: FailingGoal }
...
let rec determinismInferGoal detInfo (goal: Goal) (instMap0: InstMap) solutionContext rightFailingContexts =
    ...
    let (internalMaxSolutions0, internalCanFail) = determinismComponents internalDeterminism0
    let internalMaxSolutions = if (not (goal.Info.InstMapDelta.isReachable())) then NoSolutions else internalMaxSolutions0
    ...
    let goalInfo' = { goal.Info with Determinism = determinism }
    ...
    let finalInternalSolutions =
        ...
        | Conjunction (conjGoals) when
                        ...
                        && conjGoals
                           |> List.exists (fun conjGoal -> (determinismComponents conjGoal.Info.Determinism |> fst) = MoreThanOneSolution) ->
            MoreThanOneSolution
        | _ ->
            internalMaxSolutions

    let finalInternalDeterminism = determinismFromComponents finalInternalSolutions internalCanFail

    let needCommit =
        if (determinism <> finalInternalDeterminism) then
            match goalExpr' with
            | Disjunction _ | Scope _ -> true
            | _ -> false
        else
            false
    ...
    let goalExpr'' =
        if needCommit then
            Scope(Commit, { Goal = goalExpr'; Info = { goal.Info with Determinism = finalInternalDeterminism } })
        else
            goalExpr'

    ({ Goal = goalExpr''; Info = goalInfo' }, determinism, goalFailingContexts)

and determinismInferGoalExpr detInfo goalExpr goalInfo instMap0 solutionContext rightFailingContexts
    ...
    | Disjunction (disjGoals) ->
        let (disjGoals, determinism, failingContexts) =
            determinismInferDisjunction detInfo disjGoals instMap0 solutionContext rightFailingContexts
                CanFail NoSolutions []
        match disjGoals with
        | [] ->
            (Disjunction ([]), Fail, { Context = goalInfo.SourceInfo; Goal = FailGoal } :: failingContexts)
        | _ :: _ ->
            (Disjunction (disjGoals), determinism, failingContexts)
    | Call (callee, args) ->
        determinismInferCall detInfo callee args goalInfo solutionContext rightFailingContexts
...
and determinismInferUnify detInfo lhs rhs mode context goalInfo instMap0 solutionContext rightFailingContexts =
    let inferUnifyFailingGoal lhs rhs =
        match rhs with
        | Var (_, Assign) -> None
        | Var (rhsVar, Test) -> Some ({ Context = goalInfo.SourceInfo; Goal = TestGoal (lhs, rhsVar) })
        | Constructor (ctor, _, _, _, canFail) ->
            match canFail with
            | CanFail -> Some ({Context = goalInfo.SourceInfo; Goal = DeconstructGoal (lhs, ctor )})
            | CannotFail -> None
        | Lambda _ ->
            None
    ...
    | Some (failingGoal) ->
        (goal, determinismFromComponents OneSolution CanFail, [failingGoal])
    | None ->
        (goal, Det, [])


and determinismInferDisjunction detInfo disjGoals instMap0 solutionContext rightFailingContexts
                                            canFail numSolutions failingContexts =
    match disjGoals with
    | [] ->
    ...
    | goal :: goals ->
        ...
        let (goals', determinism, disjFailingContexts) =
            determinismInferDisjunction detInfo goals instMap0 solutionContext
                    rightFailingContexts canFail' numSolutions'' failingContexts
...
and determinismInferFSharpCall detInfo callee args retVal goalInfo solutionContext rightFailingContexts =
    ...
    if (numSolutions = CommittedChoice && solutionContext = AllSolutions) then
        failwith $"Error: call to {callee} with determinism {modes.Modes.Determinism} occurs in a context which requires all solutions"

    let goalFailingContexts =
        match canFail with
        | CanFail ->
            [{ Context = goalInfo.SourceInfo; Goal = FailingGoal.FSharpCallGoal callee }]
        | CannotFail ->
            []
...
and determinismInferCall detInfo callee args goalInfo solutionContext rightFailingContexts =
    ...
    let goalFailingContexts =
        match canFail with
        | CanFail ->
            [{ Context = goalInfo.SourceInfo; Goal = CallGoal callee }]
        | CannotFail ->
            []
...
and determinismInferConjunction detInfo conjGoals instMap0 solutionContext rightFailingContexts conjFailingContexts =
    ...
    | headGoal :: tailGoals ->
        ...
        let (tailGoals', tailDeterminism, conjFailingContexts') =
            determinismInferConjunction detInfo tailGoals instMap1 solutionContext rightFailingContexts conjFailingContexts
        let tailCanFail = determinismComponents tailDeterminism |> snd
        let headSolutionContext =
            if (tailCanFail = CannotFail && solutionContext = FirstSolution) then FirstSolution else AllSolutions
        let (headGoal', headDeterminism, headFailingContexts) =
            determinismInferGoal detInfo headGoal instMap0 headSolutionContext
...
let determinismInferProcedureBody instTable argModes varSet goal lookupRelationModes lookupFSharpModes = goal
