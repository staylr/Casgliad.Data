...
module internal InstMatch =
    ...
    let rec instMatchesFinal
        ...
        let rec instMatchesFinal3 expanded inst1 inst2 maybeType =
            ...
            | Any ->
                ...
                | BoundCtor _ ->
                    ...
                    | Some inst1' -> instMatchesFinal2 expanded inst1' inst2 maybeType
                    | None -> false
            | BoundCtor (boundInsts1) ->
                ...
                | Ground -> instTable.boundInstListIsGround (boundInsts1)
                | DefinedInst _
                | HigherOrder _
                | HigherOrderAny _
                | NotReached -> false
            | Ground ->
                ...
                | BoundCtor (boundInsts2) ->
                    ...
                    | Some instType ->
                        instTable.boundInstListIsCompleteForType (
                            (HashSet<InstName> ()),
                            boundInsts2.BoundInsts,
                            instType
                        )
            ...
            | HigherOrder info1 ->
                match inst2 with
                | HigherOrder info2
                | HigherOrderAny info2 -> higherOrderInstInfoMatches info1 info2 maybeType
                | _ -> false
        ...
        and instMatchesFinal1 (expanded: HashSet<InstMatchInputs>) inst1 inst2 maybeType =
            ...
            | (Free, Free) -> true
            | (Free, Bound _) -> false
            | (Bound boundInst1, Free) -> boundInst1 <> NotReached
            | (Bound boundInst1, Bound boundInst2) -> instMatchesFinal2 expanded boundInst1 boundInst2 maybeType
        ...
        and groundMatchesFinalBoundInstList expanded boundInsts maybeType =
            ...
            |> List.forall
                (fun boundInst ->
                    let argTypes =
                        InstTable.maybeGetConsIdArgTypes (maybeType, boundInst.Constructor)

                    List.forall2 (instMatchesFinal2 expanded Ground) boundInst.ArgInsts argTypes)
        ...
        and higherOrderInstInfoMatches (hoInst1: RelationModeE) (hoInst2: RelationModeE) maybeType : bool =
            ...
            && let argTypes =
                InstTable.maybeGetHigherOrderArgTypes (maybeType, hoInst1.Modes.Length) in
    ...
    let instMatchesBinding
        (instTable: InstTable)
        (inst1: InstE)
        (inst2: InstE)
        (maybeType: System.Type option)
        (anyMatchesAny: AnyMatchesAny)
        : bool =
        let rec instMatchesBinding3 expanded inst1 inst2 maybeType =
            ...
            | Any ->
                ...
                | Any ->
                    ...
                    | AnyDoesNotMatchAny ->
                        ...
                        | BoundCtor _ ->
                            match instTable.maybeAnyToBound (maybeType) with
                            | Some inst1' -> instMatchesBinding2 expanded inst1' inst2 maybeType
                            | None -> false
                | Ground
                | BoundCtor _ ->
                    match instTable.maybeAnyToBound (maybeType) with
            ...
            | BoundCtor (boundInsts1) ->
                ...
                | Any ->
                    match instTable.maybeAnyToBound (maybeType) with
                    | Some inst2' -> instMatchesBinding2 expanded inst1 inst2' maybeType
                    | None -> false
                | BoundCtor (boundInsts2) ->
            ...
            | HigherOrder info1 ->
                match inst2 with
                | HigherOrder info2 -> higherOrderInstInfoMatches info1 info2 maybeType
                | _ -> false
            | HigherOrderAny info1 ->
                match inst2 with
                | HigherOrderAny info2 -> higherOrderInstInfoMatches info1 info2 maybeType
        ...
        and instMatchesBinding1 (expanded: HashSet<InstMatchInputs>) inst1 inst2 maybeType =
            ...
            | (Bound boundInst1, Bound boundInst2) -> instMatchesBinding2 expanded boundInst1 boundInst2 maybeType
        ...
        and groundMatchesBindingBoundInstList expanded boundInsts maybeType =
            ...
            |> List.forall
                (fun boundInst ->
                    ...
                    List.forall2 (instMatchesBinding2 expanded Ground) boundInst.ArgInsts argTypes)
    ...
    let instMatchesInitial (instTable: InstTable) (inst1: InstE) (inst2: InstE) (maybeType: System.Type option) : bool =
        let rec instMatchesInitial3 expanded inst1 inst2 maybeType =
            ...
            | Any ->
                ...
                | BoundCtor _ ->
                    match instTable.maybeAnyToBound (maybeType) with
                    | Some inst1' -> instMatchesInitial2 expanded inst1' inst2 maybeType
                    | None -> false
            | BoundCtor (boundInsts1) ->
                match inst2 with
                | Any -> true
        ...
        and instMatchesInitial1 (expanded: HashSet<InstMatchInputs>) inst1 inst2 maybeType =
            ...
            | (Free, Bound _) -> false
            | (Bound boundInst1, Free) -> boundInst1 <> NotReached
            | (Bound boundInst1, Bound boundInst2) -> instMatchesInitial2 expanded boundInst1 boundInst2 maybeType
        ...
        and groundMatchesInitialBoundInstList expanded boundInsts maybeType =
            ...
            |> List.forall
                (fun boundInst ->
                    ...
                    List.forall2 (instMatchesInitial2 expanded Ground) boundInst.ArgInsts argTypes)
