...
module internal BitOps32 =
    ...
    let inline (==) (x: 'T) y = LanguagePrimitives.PhysicalEquality x y
    ...
    let inline branchingBit (p0: Prefix32, p1: Prefix32) : Mask32 = mostSignificantSetBit (p0 ^^^ p1)
    ...
    //
    let inline matchPrefix (key: Key32, prefix: Prefix32, mask': Mask32) : bool = mask (key, mask') = prefix

    //
    let inline zeroBit (key: Key32, mask: Mask32) : bool = key &&& mask = 0u
...
type private PatriciaSet32 =
    ...
    static member Count(set: PatriciaSet32) : int =
        ...
        | Br (_, _, left, right) ->
            ...
            PatriciaSet32.Count left
            + PatriciaSet32.Count right

    /// Retrieve the minimum element of the set.
    static member MinElement(set: PatriciaSet32) =
        match set with
        | Empty -> invalidArg "set" "The set is empty."
        | Lf j -> j
        | Br (_, _, t0, _) -> PatriciaSet32.MinElement t0
    ...
    static member MaxElement(set: PatriciaSet32) =
        ...
        | Empty -> invalidArg "set" "The set is empty."
        | Lf j -> j
        | Br (_, _, _, t1) -> PatriciaSet32.MaxElement t1
    ...
    static member Remove(key, set: PatriciaSet32) =
        match set with
        | Empty -> Empty
        | Lf j -> if j = key then Empty else set
    ...
    static member private SubsetCompare(t1: PatriciaSet32, t2: PatriciaSet32) : int =
        ...
        | Br (_, _, _, _),
          (Empty
          | Lf _) -> 1
        | Lf x, Lf y -> if x = y then 0 else 1
...
type IntSet private (trie: PatriciaSet32) =
    ...
    //
    member __.Trie
        with private get () = trie

    /// The number of elements in the IntSet.
    member __.Count: int = PatriciaSet32.Count trie
...
#endif
    member __.MinimumElement: int = int <| PatriciaSet32.MinElement trie
    ...
    [<DebuggerBrowsable(DebuggerBrowsableState.Never)>]
#endif
    member __.MaximumElement: int = int <| PatriciaSet32.MaxElement trie
    ...
    member this.Choose(chooser: int -> int option) : IntSet =
        this.Fold(
            (fun chosenSet el ->
                ...
                | None -> chosenSet
                | Some newValue -> chosenSet.Add newValue),
            IntSet.Empty
        )
        ...
        this.Fold(
            (fun (trueSet, falseSet) el ->
                if predicate el then
                    ...
                    trueSet, falseSet.Add el),
            (IntSet.Empty, IntSet.Empty)
        )

    /// Formats an element value for use within the ToString() method.
    static member private ElementString(value: int) =
    ...
    static member op_Subtraction(set1: IntSet, set2: IntSet) : IntSet = set1.Difference set2

    /// <inherit />
    override __.Equals other =
        match other with
        | :? IntSet as other -> trie = other.Trie
        | _ -> false
    ...
    interface System.IEquatable<IntSet> with
        /// <inherit />
        member __.Equals other = trie = other.Trie

    interface System.IComparable with
        /// <inherit />
        member __.CompareTo other = compare trie (other :?> IntSet).Trie

    interface System.IComparable<IntSet> with
        /// <inherit />
        member __.CompareTo other = compare trie other.Trie
    ...
    interface ICollection<int> with
        /// <inherit />
        member __.Count = PatriciaSet32.Count trie
        ...
        member this.CopyTo(array, arrayIndex) =
            ...
            if arrayIndex < 0 then
                raise
                <| System.ArgumentOutOfRangeException "arrayIndex"
...
module IntSet =
    ...
    let inline singleton (key: int) : IntSet = IntSet.Singleton key
...
module TagSet =
    ...
    /// The empty set
    [<CompiledName("Empty")>]
    let empty<[<Measure>] 'Tag> : TagSet<'Tag> = retype IntSet.Empty
    ...
    let inline minElementSigned (set: TagSet<'Tag>) : int<'Tag> =
        ...
        set.MinimumElementSigned |> retype
    ...
    let inline maxElementSigned (set: TagSet<'Tag>) : int<'Tag> =
        ...
        set.MaximumElementSigned |> retype
