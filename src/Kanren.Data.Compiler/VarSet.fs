...
type ProgVar = { Id: VarId; Name: string; VarType: System.Type }
...
type VarSet =
    ...
    with
        static member init = { NextVar = 0<varIdMeasure>; Vars = Map.empty; VarNames = Map.empty; QVars = Map.empty }

        member v.newNamedVar(varName: string, varType: System.Type) : (VarSet * ProgVar) =
            let varId = v.NextVar
            let rec makeUniqueName name = if (v.VarNames.ContainsKey(name)) then makeUniqueName $"{name}__{varId}" else name
            let progVar = { Id = v.NextVar; Name = makeUniqueName varName; VarType = varType }
        ...
        member v.addQuotationVar(qvar) =
            if (v.QVars.ContainsKey(qvar)) then
                ...
                ({ v' with QVars = v'.QVars.Add(qvar, var)}, var)
